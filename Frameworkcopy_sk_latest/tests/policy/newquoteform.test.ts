...
test.describe('Class Code And Risk Builder', () => {
  ...
  test('Fill class code, zip, address and click Add', async ({ page }) => {
    ...
    await page.locator('.simple-typeahead-list-item').first().waitFor({ state: 'visible', timeout: 30000 });
...
test.describe('Location and Risk Details', () => {
  ...
  test('Fill crime index and exposure → verify final rate and premium calculated', async ({ page }) => {
    ...
    // Wait for exposure input to be enabled (page may still be processing after class code add)
    await expect(page.locator('input#exposureAmountInput')).toBeEnabled({ timeout: 30000 });

    // Fill Exposure Amount then press Enter to trigger calculation
    ...
    await expect(page.locator('input#classBuilderFinalRate')).not.toHaveValue('', { timeout: 15000 });

    // Verify Final Premium is calculated (not empty)
    await expect(page.locator('input#classBuilderPremium')).not.toHaveValue('', { timeout: 15000 });
  });

  test('Click + on L1 → R2 appears in table', async ({ page }) => {
    await page.locator('#gllocationTable #addRisk').first().scrollIntoViewIfNeeded();
    await page.locator('#gllocationTable #addRisk').first().click();
    await page.waitForLoadState('networkidle', { timeout: 15000 });
    await expect(page.locator('#gllocationTable').getByText('R2')).toBeVisible({ timeout: 10000 });
  });

  test('Fill R2 class code 91111 and exposure → verify final rate and premium', async ({ page }) => {
    // Fill class code 91111 for R2 via typeahead
    const r2Row = page.locator('#gllocationTable tr').filter({ hasText: 'R2' }).first();
    await r2Row.locator('input[placeholder="Class Code"]').scrollIntoViewIfNeeded();
    await r2Row.locator('input[placeholder="Class Code"]').click();
    await r2Row.locator('input[placeholder="Class Code"]').pressSequentially(testData[0].r2ClassCode, { delay: 100 });
    await page.locator('.simple-typeahead-list-item').first().waitFor({ state: 'visible', timeout: 30000 });
    await page.locator('.simple-typeahead-list-item').first().evaluate(el => (el as HTMLElement).click());
    await page.waitForTimeout(800);

    // Fill R2 exposure from CSV and press Enter to trigger calculation
    await r2Row.locator('input#exposureAmountInput').fill(testData[0].r2ExposureAmount);
    await page.keyboard.press('Enter');
    await page.waitForLoadState('networkidle', { timeout: 30000 });
    await page.waitForTimeout(1000);

    // Verify final rate and premium are calculated for R2
    await expect(r2Row.locator('input#classBuilderFinalRate')).not.toHaveValue('', { timeout: 15000 });
    await expect(r2Row.locator('input#classBuilderPremium')).not.toHaveValue('', { timeout: 15000 });
  });

  test('Add L2 - fill class code, zip, address and click Add', async ({ page }) => {
    await page.locator('input.glClassCodeBuilder[placeholder="Class Code"]').scrollIntoViewIfNeeded();
    await page.locator('input.glClassCodeBuilder[placeholder="Class Code"]').click();
    await page.locator('input.glClassCodeBuilder[placeholder="Class Code"]').selectText();
    ...
    await page.locator('input.creatorZipVal').click();
    await page.locator('input.creatorZipVal').selectText();
    ...
    await expect(page.locator('#gllocationTable').getByText('L2', { exact: true })).toBeVisible({ timeout: 30000 });
  });

  test('Fill L2 R1 crime index and exposure → verify final rate and premium', async ({ page }) => {
    const l2Row = page.locator('#gllocationTable tr')
      .filter({ hasText: 'R1' })
      .filter({ has: page.locator('input#exposureAmountInput') })
      .nth(1);

    await page.waitForLoadState('networkidle', { timeout: 60000 });
    await expect(l2Row.locator('input#classBuilderCrimeIndex')).toBeEnabled({ timeout: 60000 });
    await l2Row.locator('input#classBuilderCrimeIndex').scrollIntoViewIfNeeded();
    await l2Row.locator('input#classBuilderCrimeIndex').fill(testData[0].l2crimeindex);
    await page.waitForTimeout(500);

    await page.waitForLoadState('networkidle', { timeout: 60000 });
    await expect(l2Row.locator('input#exposureAmountInput')).toBeEnabled({ timeout: 60000 });
    await l2Row.locator('input#exposureAmountInput').click();
    await l2Row.locator('input#exposureAmountInput').fill(testData[0].l2ExposureAmount);
    await page.keyboard.press('Enter');
    await page.waitForLoadState('networkidle', { timeout: 60000 });
    await page.waitForTimeout(3000);

    await expect(l2Row.locator('input#classBuilderFinalRate')).not.toHaveValue('', { timeout: 60000 });
    await expect(l2Row.locator('input#classBuilderPremium')).not.toHaveValue('', { timeout: 60000 });
  });

  test('L2 R1 - enable If Any and fill minimum premium', async ({ page }) => {
    const l2Row = page.locator('#gllocationTable tr')
      .filter({ hasText: 'R1' })
      .filter({ has: page.locator('input#exposureAmountInput') })
      .nth(1);

    await l2Row.locator('label[for*="glIfany"]').scrollIntoViewIfNeeded();
    await l2Row.locator('label[for*="glIfany"]').click();
    await page.waitForTimeout(500);

    await l2Row.locator('input.minpremsystem').fill(testData[0].l2r1minimumpremium);
    await page.keyboard.press('Tab');
    await page.waitForTimeout(500);
  });

  test('Add L3 - fill class code, zip, address and click Add', async ({ page }) => {
    await page.locator('input.glClassCodeBuilder[placeholder="Class Code"]').scrollIntoViewIfNeeded();
    await page.locator('input.glClassCodeBuilder[placeholder="Class Code"]').click();
    await page.locator('input.glClassCodeBuilder[placeholder="Class Code"]').selectText();
    ...
    await expect(page.locator('#gllocationTable').getByText('L3', { exact: true })).toBeVisible({ timeout: 10000 });
  });

  test('Fill L3 R1 crime index and exposure → verify final rate and premium', async ({ page }) => {
    const l3HeaderRow = page.locator('#gllocationTable tr').filter({ hasText: 'L3' }).first();
    const l3Row = page.locator('#gllocationTable tr')
      .filter({ hasText: 'R1' })
      .filter({ has: page.locator('input#exposureAmountInput') })
      .nth(2);

    await l3HeaderRow.locator('input#classBuilderCrimeIndex').scrollIntoViewIfNeeded();
    await l3HeaderRow.locator('input#classBuilderCrimeIndex').fill(testData[0].l3crimeindex);
    await page.waitForTimeout(500);

    await expect(l3Row.locator('input#exposureAmountInput')).toBeEnabled({ timeout: 30000 });
    await l3Row.locator('input#exposureAmountInput').fill(testData[0].l3classcodeexposure);
    await page.keyboard.press('Enter');
    await page.waitForLoadState('networkidle', { timeout: 30000 });
    await page.waitForTimeout(1000);

    await expect(l3Row.locator('input#classBuilderFinalRate')).not.toHaveValue('', { timeout: 15000 });
    await expect(l3Row.locator('input#classBuilderPremium')).not.toHaveValue('', { timeout: 15000 });
  });

  test('Add L4 - fill class code, zip, address and click Add', async ({ page }) => {
    await page.locator('input.glClassCodeBuilder[placeholder="Class Code"]').scrollIntoViewIfNeeded();
    await page.locator('input.glClassCodeBuilder[placeholder="Class Code"]').click();
    await page.locator('input.glClassCodeBuilder[placeholder="Class Code"]').selectText();
    ...
    await expect(page.locator('#gllocationTable').getByText('L4', { exact: true })).toBeVisible({ timeout: 10000 });
  });

  test('Fill L4 R1 exposure → verify final rate and premium', async ({ page }) => {
    const l4Row = page.locator('#gllocationTable tr')
      .filter({ hasText: 'R1' })
      .filter({ has: page.locator('input#exposureAmountInput') })
      .nth(3);
    await l4Row.locator('input#exposureAmountInput').scrollIntoViewIfNeeded();
    await expect(l4Row.locator('input#exposureAmountInput')).toBeEnabled({ timeout: 30000 });
    await l4Row.locator('input#exposureAmountInput').fill(testData[0].l4classcodeexposure);
    await page.keyboard.press('Enter');
    await page.waitForLoadState('networkidle', { timeout: 30000 });
    await page.waitForTimeout(1000);

    await expect(l4Row.locator('input#classBuilderFinalRate')).not.toHaveValue('', { timeout: 15000 });
    await expect(l4Row.locator('input#classBuilderPremium')).not.toHaveValue('', { timeout: 15000 });
  });

  test('L4 R1 - enable If Any and fill Override Premium', async ({ page }) => {
    const l4Row = page.locator('#gllocationTable tr')
      .filter({ hasText: 'R1' })
      .filter({ has: page.locator('input#exposureAmountInput') })
      .nth(3);

    await l4Row.locator('label[for*="glIfany"]').scrollIntoViewIfNeeded();
    await l4Row.locator('label[for*="glIfany"]').click();
    await page.waitForTimeout(500);

    const overridePremiumInput = l4Row.locator('input#glOverrideModInput');
    await overridePremiumInput.waitFor({ state: 'enabled', timeout: 10000 });
    await overridePremiumInput.fill(testData[0].l4OverridePremium);
    await page.keyboard.press('Tab');
    await page.waitForTimeout(500);
  });

  test('Clone L2 R1 → L5 appears in table', async ({ page }) => {
    const l2Row = page.locator('#gllocationTable tr').filter({ hasText: 'L2' }).first();
    await l2Row.locator('span[title="Clone"]').scrollIntoViewIfNeeded();
    await l2Row.locator('span[title="Clone"]').click();
    await page.waitForLoadState('networkidle', { timeout: 15000 });
    await expect(page.locator('#gllocationTable').getByText('L5', { exact: true })).toBeVisible({ timeout: 10000 });
  });

  test('Verify L5 R1 cloned values - final rate and premium already filled', async ({ page }) => {
    const l5Row = page.locator('#gllocationTable tr')
      .filter({ hasText: 'R1' })
      .filter({ has: page.locator('input#exposureAmountInput') })
      .nth(4);
    await l5Row.locator('input#exposureAmountInput').scrollIntoViewIfNeeded();

    await expect(l5Row.locator('input#exposureAmountInput')).not.toHaveValue('', { timeout: 10000 });
    await expect(l5Row.locator('input#classBuilderFinalRate')).not.toHaveValue('', { timeout: 10000 });
    await expect(l5Row.locator('input#classBuilderPremium')).not.toHaveValue('', { timeout: 10000 });
  });

  test('Verify L5 R1 cloned If Any and minimum premium already filled', async ({ page }) => {
    const l5Row = page.locator('#gllocationTable tr')
      .filter({ hasText: 'R1' })
      .filter({ has: page.locator('input#exposureAmountInput') })
      .nth(4);
    await l5Row.locator('input.minpremsystem').scrollIntoViewIfNeeded();

    await expect(l5Row.locator('input.minpremsystem')).not.toHaveValue('', { timeout: 10000 });
  });
});
